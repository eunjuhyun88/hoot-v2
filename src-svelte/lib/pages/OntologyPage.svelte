      ...
      <section class="section">
        ...
        <div class="section-body branches-list">
          {#each BRANCH_CATALOG as def}
            ...
            {#if branch}
              <div class="branch-card" class:disabled={!branch.enabled} class:expanded={expandedBranch === def.id}>
                <div class="branch-header">
                  <button class="branch-toggle" on:click={() => toggleBranch(def.id)}>
                  ...
                  <div class="branch-iters">
                    <input
                      ...
                      max="200"
                      value={branch.iters}
                      on:change={(e) => updateBranchIters(def.id, parseInt(e.currentTarget.value) || def.defaultIters)}
                  ...
                  <button
                    class="expand-btn"
                    on:click={() => toggleBranchExpand(def.id)}
                    disabled={!branch.enabled}
                  >
                ...
                {#if expandedBranch === def.id && branch.enabled}
                      ...
                      <div class="chip-grid">
                        {#each def.models as model}
                          <button
                            ...
                            on:click={() => toggleModel(def.id, model.id)}
                      ...
                      <div class="chip-grid">
                        {#each def.featureCategories as cat}
                          <button
                            ...
                            on:click={() => toggleCategory(def.id, cat.id)}
                      ...
                      <div class="chip-grid">
                        {#each def.strategies as s}
                          <button
                            ...
                            on:click={() => toggleArrayItem(def.id, 'selectedStrategies', s)}
                          >{s}</button>
                        {/each}
                      </div>
                      ...
                      <div class="chip-grid">
                        {#each def.methods as m}
                          <button
                            ...
                            on:click={() => toggleArrayItem(def.id, 'selectedMethods', m)}
                          >{m}</button>
                        {/each}
                      </div>
                      ...
                      <div class="chip-grid">
                        {#each def.optimizers as opt}
                          <button
                            ...
                            on:click={() => {
                              ontology.branches = ontology.branches.map(b =>
                                b.type === def.id ? { ...b, selectedOptimizer: opt } : b
                              );
                            }}
                          >{opt}</button>
                        {/each}
                      ...
                      <div class="chip-grid">
                        {#each def.transforms as t}
                          <button
                            ...
                            on:click={() => toggleArrayItem(def.id, 'selectedTransforms', t)}
                          >{t}</button>
                        {/each}
                      </div>
                    {/if}
